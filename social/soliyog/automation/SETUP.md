...
Telegram needs no OAuth or app review — a bot token posts to a channel directly.

1. Message **@BotFather** on Telegram → `/newbot` → follow the prompts → copy the token
   it gives you as `TELEGRAM_BOT_TOKEN`.
2. Create the channel (e.g. "Soliyog Job Updates") if you haven't already.
3. Add the bot as an **admin** of the channel, with at least "Post Messages" permission
   (Channel → Administrators → Add Admin → search the bot by its `@username`).
4. `TELEGRAM_CHANNEL_ID` is the channel's `@username` if it's public, or its numeric
   `-100...` chat id if private — `verify-setup.mjs` will validate either.

Unlike the other three platforms, Telegram is **soft-optional**: `post.mjs` posts to it
by default (see §"Daily use"), but a missing `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHANNEL_ID`
just skips it with a warning — it does not fail the FB/IG/LinkedIn part of the run.

## 2d. Telegram batch feed — a second, higher-volume post to the same channel
...
- LinkedIn posts by default alongside FB/IG (`platforms: [instagram, facebook, linkedin,
  telegram]` in every new queue item, via `new-post.mjs`). `caption_linkedin` is already
  generated by `build-caption.mjs` alongside the others. Needs `BUFFER_TOKEN` +
  `BUFFER_LINKEDIN_CHANNEL_ID` set (see §2b) — opt a specific item out with
  `platforms: [instagram, facebook]`.
- Telegram also posts by default (see §2c for bot/channel setup). Unlike LinkedIn, an
  unconfigured `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHANNEL_ID` doesn't need an opt-out —
  it's silently skipped with a warning rather than failing the run. Use
  `platforms: [instagram, facebook, linkedin]` on a specific item only if you want to
  skip Telegram for that one post even after it's configured.
